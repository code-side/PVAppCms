...
@Component({
    selector: 'jhi-tourist-destination',
    templateUrl: './tourist-destination.component.html'
})
export class TouristDestinationComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.touristDestinationService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/tourist-destination'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: TouristDestination) {
        return item.id;
    }
    registerChangeInTouristDestinations() {
        this.eventSubscriber = this.eventManager.subscribe('touristDestinationListModification', (response) => this.loadAll());
    }
